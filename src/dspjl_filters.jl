...
function dspjl_cr_filter(RC::Real)
    T = float(typeof(RC))
    α = RC / (RC + 1)
...
function dspjl_inv_cr_filter(RC::Real)
    T = float(typeof(RC))
    α = 1 / (1 + RC)
    k = 1 + 1/RC
...
function dspjl_crmod_filter(RC::Real)
    T = float(typeof(RC))
    α = RC / (RC + 1)
...
function dspjl_inv_crmod_filter(RC::Real)
    T = float(typeof(RC))
    α = 1 / (1 + RC)
...
function dspjl_integrator_filter(gain::Real)
    T = float(typeof(gain))
...
function dspjl_integrator_crmod_filter(gain::Real, RC::Real)
    T = float(promote_type(typeof(gain), typeof(RC)))
    α = 1 / (1 + RC)
